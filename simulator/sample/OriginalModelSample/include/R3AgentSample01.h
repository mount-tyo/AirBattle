...
DECLARE_CLASS_WITHOUT_TRAMPOLINE(R3AgentSample01,SingleAssetAgent)
	...
	1. 観測データについて
		...
		* 各フレームの観測データの内訳
			...
			2. 味方機情報
				...
				2. 速度・・・自機と同じ
				3.残弾数・・・自機と同じ
			3. 敵機情報・・・見えている敵機航跡のうち、近いものから順に最大maxTrackNum["Enemy"]機分。無い場合は0埋め。
				1. 位置・・・味方機と同じ
				2. 速度・・・味方機と同じ
			4. 味方誘導弾情報(自機も含む)・・・飛翔中の誘導弾のうち、射撃時刻が古いものから順に最大maxMissileNum["Friend"]発分。無い場合は0埋め。
				1. 位置・・・味方機と同じ
				2. 速度・・・自機と同じ
				3. 誘導状態・・・guided,self,memoryの3通りについて、one-hot形式で与える。
			5. 敵誘導弾情報・・・見えている誘導弾航跡のうち、近いものから順に最大maxMissileNum["Enemy"]発分。無い場合は0埋め。
	...
	*/
    ...
    //内部変数
	int singleDim;
	VecX<long> lastAction;
    std::vector<Track3D> lastTrackInfo;
    Eigen::Vector3d dstDir,omegaScale;
    ...
    public:
    R3AgentSample01(const nl::json& modelConfig_,const nl::json& instanceConfig_);
    virtual ~R3AgentSample01();
